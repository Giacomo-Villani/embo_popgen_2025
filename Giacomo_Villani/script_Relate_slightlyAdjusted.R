...
folder.fastSimcoal2 <- "~/embo_popgen_2025/Giacomo_Villani";
...
{
  ex <- file.path(folder.fastSimcoal2, "DemographicModelSplitR", "DemographicModelSplitR_mrca.txt");
  if (file.exists(ex)) {
    file.remove(ex)
  }  
...
plot(x=time.change, y=mean.log.tmrca)
...
ne_bottleneck <- 500
time_change_population_size <- 51
time_duration_bottleneck <- 50
number_of_blocks <- 10000

model.single.pop.with.recent.population.change.tMRCA <- function(population_size_in_present, effective_population_size_1, time_change_population_size, number_of_blocks, time_duration_bottleneck)
{
  ex <- file.path(folder.fastSimcoal2, "DemographicModelSplitR", "DemographicModelSplitR_mrca.txt");
  if (file.exists(ex)) {
    file.remove(ex)
...
data <- model.single.pop.with.recent.population.change.tMRCA(population_size_in_present, ne_bottleneck, time_change_population_size, time_duration_bottleneck, number_of_blocks)
iicr(data)
